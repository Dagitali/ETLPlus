...
Local hooks in `.pre-commit-config.yaml` complement this policy, but GitHub
rulesets are the authoritative enforcement layer.

## Recommended Required Checks

Use one simplified required-check baseline for both `main` and `develop`.

Because `.github/workflows/ci.yml` uses matrices for Python versions, docs
builders, and operating systems, GitHub exposes the required checks as the
expanded matrix job names rather than the template names shown in the YAML.
Select the exact expanded names in the GitHub ruleset UI.
...
In the current CI workflow, that baseline resolves to these exact check names:
...
These checks map to stable CI job names in `.github/workflows/ci.yml` and are
intended to be the minimum merge gate for protected branches.

The remaining CI jobs are still useful, but they should usually stay advisory
rather than blocking unless you intentionally want a stricter gate:
...
If you want a stricter protected-branch gate, the natural next checks to add
are:
...
That keeps the matrix-driven coverage intact without abandoning the DRY CI
layout.
...
- If merge queue is enabled, keep the `merge_group` trigger in CI so the same
  checks run for queued merges
...
- Require branches to be up to date before merging
- Add the required checks listed above
- If merge queue is enabled on `develop`, keep the `merge_group` trigger in CI
  so the same checks run for queued merges
...
- Require merge queue if `develop` receives enough concurrent PR traffic to make
...
- GitHub required checks are tied to the exact job names emitted by the CI
  workflow after matrix expansion. In this repository, that means the ruleset
  should reference concrete names such as `Lint on Python 3.13` rather than the
  template string `Lint on Python ${{ matrix.python-version }}` shown in the
  YAML.
- Treat version-specific and OS-specific names in this document as current
  examples, not permanent policy. When the support matrix changes, refresh the
  exact examples here and in the GitHub ruleset UI to match the emitted checks.
- The `Build distributions` job is conditionally skipped for pull requests from
  forks. If fork-origin pull requests are a normal part of your workflow,
  confirm how your GitHub ruleset treats skipped required checks before making
  `Build distributions` mandatory, or relax that requirement for branches that
  regularly accept external fork PRs.
- The local `no-commit-to-branch` pre-commit hook should protect `main` and
  `develop`, but it is only a contributor convenience. GitHub rulesets remain
  authoritative.
