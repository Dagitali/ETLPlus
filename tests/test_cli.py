...
from etlplus.__main__ import create_parser, main
import sys
...
def test_parser_extract_command():
    ...
    parser = create_parser()
    args = parser.parse_args(["extract", "file", "/path/to/file.json"])
...
def test_parser_load_command():
    ...
    parser = create_parser()
    args = parser.parse_args(["load", "/path/to/file.json", "file", "/path/to/output.json"])
...
def test_main_extract_file(monkeypatch, capsys):
    ...
    try:
        monkeypatch.setattr(sys, "argv", ["etlplus", "extract", "file", temp_path])
        result = main()
        assert result == 0
        captured = capsys.readouterr()
...
def test_main_transform_data(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["etlplus", "transform", json_data, "--operations", operations])
    result = main()
    assert result == 0
    captured = capsys.readouterr()
    output_data = json.loads(captured.out)
...
def test_main_extract_with_output(monkeypatch, capsys):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            monkeypatch.setattr(sys, "argv", [
                "etlplus", "extract", "file", temp_path, "-o", str(output_path)
            ])
            result = main()
            assert result == 0
            assert output_path.exists()
...
def test_main_error_handling(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["etlplus", "extract", "file", "/nonexistent/file.json"])
    result = main()
    assert result == 1
    captured = capsys.readouterr()
